...
public class Helper {
	...
	public static HashMap<Integer, Long> powerOfTwo = null;
	...
	public static String requestShare(InetSocketAddress server, String req) {
		...
		try {
			talkSocket = new Socket(server.getAddress(),server.getPort()+1);
	...
	public static String sendRequest(InetSocketAddress server, String req) {
		...
		if (server == null || req == null) {
			if(server == null) {
				new Exception("server == null").printStackTrace();
				return null;
			}
			if(req == null) {
				new Exception("req == null").printStackTrace();
				return null;
			}
		}
